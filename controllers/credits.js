...
exports.getCredits = asyncHandler(async (req, res) => {
  try {
    ...
    const roundedCredits =
      Math.round(((entity.credits || 0) + Number.EPSILON) * 100) / 100;
...
exports.adminManageCredits = asyncHandler(async (req, res) => {
  ...
  try {
    const { userId, providerId, action, amount, description, reference } =
      req.body;
    ...
    res.status(200).json({
      ...
      message: `${roundedAmount} credits ${
        action === "add"
          ? "added to"
          : action === "use"
          ? "deducted from"
          : "refunded to"
      } ${entityType} ${updatedEntity.name}`,
    });
  } catch (error) {
    // Abort transaction on error
    if (session) {
...
// @desc    Pay for rental using credits and transfer to provider
...
exports.payRentalWithCredits = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Find the rental with car and provider data
        ...
        // Check if car and provider exist
        if (!rental.car) {
            await session.abortTransaction();
            return res.status(404).json({
                success: false,
                message: 'Car information not found for this rental'
            });
        }
        
        // Get provider ID from car
        ...
        // Initialize provider credits if needed
        ...
        // 3. Create payment transaction for user (negative amount)
        ...
        // 4. Create receipt transaction for provider (positive amount)
        // It seems the Transaction model requires a user field even for provider transactions
        const providerTransaction = await Transaction.create(
            [{
                ...
                status: 'completed',
                provider: providerId,
                user: auth.id, // Include the user ID to satisfy the schema requirement
                metadata: {
                    rentalDetails: {
                        startDate: rental.startDate,
                        returnDate: rental.returnDate,
        ...
        // 5. Update rental status to 'completed'
        ...
        res.status(200).json({
            ...
            message: `Rental paid successfully with ${amountToPay} credits. Payment transferred to provider.`
        });
...
exports.getAllTransactions = asyncHandler(async (req, res) => {
    try {
        ...
        // Aggregate summary statistics for ALL transactions (no filters)
        const [
            deposits, 
            payments, 
            refunds, 
            ...
            Transaction.aggregate([
                { $match: { type: 'deposit' } },  // Only filter by type
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ]),
            Transaction.aggregate([
                { $match: { type: 'payment' } },  // Only filter by type
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ]),
            Transaction.aggregate([
                { $match: { type: 'refund' } },  // Only filter by type
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ]),
            Transaction.aggregate([
                { $match: { type: 'withdrawal' } },  // Only filter by type
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ])
        ]);
  ...
  try {
    ...
    const isOwner =
      ...
      (transaction.provider &&
        transaction.provider.toString() === userId.toString());
...
(exports.getUserTransactionHistory = asyncHandler(async (req, res) => {
  try {
    ...
    // Create a query that only filters by entity for summary calculation
    const summaryQuery = {};
    summaryQuery[auth.type] = auth.id;

    // Get ALL transactions for this entity (unfiltered)
    const allEntityTransactions = await Transaction.find(summaryQuery);
    console.log("Total Entity Transactions for Summary:", allEntityTransactions.length);

    // Manual summary calculation for ALL transactions
    const summary = {
    ...
    allEntityTransactions.forEach((transaction) => {
